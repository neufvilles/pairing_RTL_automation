        ...
        if (operand in input_value):
            ...
            if opcode == "FP2MUL":
                ...
                if pre_resource0 is None:
                    f_write.write("\t{0} += alt(FP2MUL_mem)\n".format(mem_value_name))
                    for j in range(MULnum):
                        if out_fp_number == 2:
                            f_write.write("\tS += ({0}*FP2MUL[{3}])-1 < {1}_mem{2}*FP2MUL_mem[{3}]\n".format(operand, value, i, j))
                        else:
                            f_write.write("\tS += ({0}*FP2MUL[{3}])-1 < {1}_mem{2}*FP2MUL_mem[{3}]\n".format(operand[:-1], value, i, j))
                else:
                    if prev_formula1 is None:
                        f_write.write("\t{0} += FP2MUL_mem[{1}]\n".format(mem_value_name, pre_resource_num0))
                        ...
                        if pre_resource1 is None:
                            ...
                            f_write.write("\t{0} += FP2MUL_mem[{1}], FP2MUL_mem[{2}]\n".format(mem_value_name, pre_resource_num0, pre_resource_num1))
                            f_write.write("\tS += {1} < {0}, {2} < {0}\n".format(mem_value_name, pre_end_time0 - 1, pre_end_time1 - 1))                            
                ...
                if pre_resource0 is None:
                    ...
                    if prev_formula1 is None:
                        ...
                        if pre_resource1 is None:
                            ...
                            f_write.write("\t{0} += CONSTMUL_mem[{1}], CONSTMUL_mem[{2}]\n".format(mem_value_name, pre_resource_num0, pre_resource_num1))
                            f_write.write("\tS += {1} < {0}, {2} < {0}\n".format(mem_value_name, pre_end_time0 - 1, pre_end_time1 - 1))
            elif opcode == "ADD" or opcode == "SUB":
                if pre_resource0 is None:
                    if out_fp_number == 2:
                        ab_count = True
                        f_write.write("\t{0}a = S.Task('{0}a', length=1, delay_cost=1)\n".format(mem_value_name))
                        f_write.write("\t{0}a += alt(ADD_mem)\n".format(mem_value_name))
                        for j in range(ADDnum):
                            f_write.write("\tS += ({0}0*ADD[{3}])-1 < {1}_mem{2}a*ADD_mem[{3}]\n".format(operand, value, i, j))
                        f_write.write("\t{0}b = S.Task('{0}b', length=1, delay_cost=1)\n".format(mem_value_name))
                        if pre_resource1 is None:
                            f_write.write("\t{0}b += alt(ADD_mem)\n".format(mem_value_name))
                            for j in range(ADDnum):
                                f_write.write("\tS += ({0}1*ADD[{3}])-1 < {1}_mem{2}b*ADD_mem[{3}]\n".format(operand, value, i, j))
                        else:
                            f_write.write("\t{0}b += ADD_mem[{1}]\n".format(mem_value_name, pre_resource_num1))
                            f_write.write("\tS += {1} < {0}b\n".format(mem_value_name, pre_end_time1 - 1))
                    else:
                        f_write.write("\t{0} = S.Task('{0}', length=1, delay_cost=1)\n".format(mem_value_name))
                    ...
                    if out_fp_number == 1:
                        ...
                        # f_write.write("# why\t{0}a\n".format(mem_value_name))
                        ab_count = True
                        f_write.write("\t{0}a = S.Task('{0}a', length=1, delay_cost=1)\n".format(mem_value_name))
                        f_write.write("\t{0}a += ADD_mem[{1}]\n".format(mem_value_name, pre_resource_num0))
                        f_write.write("\tS += {1} < {0}a\n".format(mem_value_name, pre_end_time0 - 1))
                        f_write.write("\t{0}b = S.Task('{0}b', length=1, delay_cost=1)\n".format(mem_value_name))
                        if pre_resource1 is None:
                            f_write.write("\t{0}b += alt(ADD_mem)\n".format(mem_value_name))
                            for j in range(ADDnum):
                            ...
                            f_write.write("\tS += {1} < {0}b\n".format(mem_value_name, pre_end_time1 - 1))                        
            if ab_count is True:
                f_write.write("\tS += {1}a <= {0}\n".format(value, mem_value_name))
                f_write.write("\tS += {1}b <= {0}\n\n".format(value, mem_value_name))
            else:
                f_write.write("\tS += {1} <= {0}\n\n".format(value, mem_value_name))
