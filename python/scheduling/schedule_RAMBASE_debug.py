                ...
                if opcode == "CONSTMUL":
                    if pre_resource0 is None:
                        ...
                        for j in range(CONSTnum):
                            f_write.write("\tS += ({0}*COSNTMUL[{3}])-1 < {1}_mem{2}*CONSTMUL_mem[{3}]\n".format(operand, value, i, j))
                    else:
                    ...
                    if pre_resource0 is None:
                        ...
                        for j in range(ADDnum):
                            f_write.write("\tS += ({0}*ADD[{3}])-1 < {1}_mem{2}[0]*ADD_mem[{3}]\n".format(operand, value, i, j))
                    else:
                    ...
                    if pre_resource1 is None:
                        ...
                        for j in range(ADDnum):
                            f_write.write("\tS += ({0}*ADD[{3}])-1 < {1}_mem{2}[1]*ADD_mem[{3}]\n".format(operand, value, i, j))
                    else:
            ...
            if operand in input_value:
                ...
                if opcode == "FP2MUL":
                    if pre_resource is None:
                        ...
                        f_write.write("\t{0} += FP2MUL_mem{2}[{1}]\n".format(mem_value_name, pre_resource_num0, operand_num))
                        f_write.write("\tS += {1} < {0}\n".format(mem_value_name, pre_end_time0 - 1))
                    f_write.write("\t{0}[0] += {0}[1]*FP2MUL_mem{1}\n".format(mem_value_name, operand_num))
...
def make_pyschedule(
    ...
    # 1write-2read RAM
    f_write.write("\tFP2MUL_mem = S.Resources('FP2MUL_mem', num=2*{0}, size=2)\n".format(MULnum)) #2r1w * 2
    ...
    multi_resources = ["FP2MUL", "FP2MUL_in", "CONSTMUL", "ADD", "FP2MUL_mem", "ADD_mem"]
    single_resources = ["INV", "INPUT_mem_w", "INPUT_mem_r", "CONSTMUL_mem"]
